...
function removeSubpaths(paths) {
  return paths.filter((path, index) => 
    !paths.some((otherPath, otherIndex) => 
      otherIndex !== index && isSubpath(otherPath, path)
    )
  );
}
function pathDFS(cell:Cell,path:[],paths:[],graph:Graph){
    path.push(cell.id)
    if(path.length > 1) paths.push([...path])
    const neighbors = getNeighbors(graph,cell)
    for (const neighbor of neighbors) {
      if (!path.includes(neighbor.getId())) {
        pathDFS(neighbor,path,paths,graph);
      }
    }
  path.pop();
  return paths;
...
function findAndRemoveDefaultEdges(graph: Graph, pointsMap) {
  ...
  const filteredArr = Object.values(graph.model.cells).filter(item => item.hasOwnProperty('isType'));
  let usefulPath = []
  for(const item of filteredArr){
   const graphPath  =  pathDFS(item,[],[],graph)
   usefulPath.push(...graphPath)
  }
  console.log(usefulPath,'useful')
  const result = removeSubpaths(usefulPath)
...
drawAxes(graph);
  ...
  function addVertex(icon: string, w: number, h: number, style: CellStyle, title: string) {
    const vertex = new Cell(title, new Geometry(0, 0, w, h), style)as CustomCell;
      vertex.isType =title; 
  ...
  style.strokeColor = '#000000'
    ...
    try {
      ...
      dataList.point.forEach(point => {
        ...
        const vertex =  graph.insertVertex(parent, null, point.id, adjustedX, adjustedY, vertexWidth, vertexHeight, iniuStyle)as CustomCell;
...
graph.addListener(InternalEvent.MOUSE_DOWN, (sender, evt) => {
...
try {
  ...
  for (let i = 0; i <= xAxisLength; i += xAxisStep) {
    ...
    const vertex =  graph.insertVertex(parent, null, label, x, containerHeight - 10, 1, 10, {strokeColor: '#000000', strokeWidth: 1, fillColor: '#000000',movable:false})as CustomCell;
     vertex.isAxis = true; 
  }
  ...
  for (let i = 0; i <= yAxisLength; i += yAxisStep) {
    ...
    const vertex =  graph.insertVertex(parent, null, label, 10, containerHeight - y, 10, 1, {strokeColor: '#000000', strokeWidth: 1, fillColor: '#000000',movable:false})as CustomCell;
     vertex.isAxis = true;
  }
} finally {
  model.endUpdate();
...
function addMouseWheelZoom(graph: Graph, container: HTMLDivElement) {
  ...
  container.addEventListener('wheel', (evt: WheelEvent) => {
    ...
    graph.view.scaleAndTranslate(
      currentScale,
      0,
      evt.deltaY < 0? graph.view.translate.y - newTranslateY + 5:graph.view.translate.y - newTranslateY - 5
    );
      ...
      if (e.key === 'Backspace' || e.key === 'Delete') {
        ...
        if(selectionCell&&(selectionCell.isEdge() || selectionCell.isVertex())){
          graphs.value.getDataModel().beginUpdate();
            try {
              if(selectionCell.style.deletable === false || selectionCell.style.dashed === true) return
...
// 找出 b 中在 a 中不存在的子数组
const missingInA = initPaths.filter(subArrayB => 
    !pathB.some(subArrayA => arraysEqual(subArrayA, subArrayB))
);
